...
interface SystemSettingsProps {
    onThemeUpdate?: () => void; // New callback prop
}

export const SystemSettings: React.FC<SystemSettingsProps> = ({ onThemeUpdate }) => {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Trigger update in main App to refresh Theme
  ...
  return (
    <div className="space-y-8 pb-10 text-right font-tajawal">
      ...
      <div className="flex flex-col md:flex-row justify-between items-center gap-4 bg-white p-6 rounded-[2rem] border border-gray-100 shadow-sm">
        <div>
          <h2 className="text-3xl font-black text-primary">إعدادات المنصة</h2>
          <p className="text-sm text-gray-400 font-bold mt-1">إدارة شاملة للمظهر، المحتوى، والمالية.</p>
        </div>
        <div className="flex flex-col md:flex-row gap-4 items-center w-full md:w-auto">
            <div className="flex gap-2 bg-gray-50 p-1 rounded-xl border border-gray-100 overflow-x-auto no-scrollbar max-w-sm md:max-w-none">
                <button onClick={() => setActiveTab('general')} className={`px-4 py-2 rounded-lg text-xs font-bold whitespace-nowrap transition-all ${activeTab === 'general' ? 'bg-primary text-white' : 'text-gray-500 hover:bg-white'}`}>عام ورسوم</button>
                <button onClick={() => setActiveTab('theme')} className={`px-4 py-2 rounded-lg text-xs font-bold whitespace-nowrap transition-all ${activeTab === 'theme' ? 'bg-primary text-white' : 'text-gray-500 hover:bg-white'}`}>المظهر والهوية</button>
                <button onClick={() => setActiveTab('booking')} className={`px-4 py-2 rounded-lg text-xs font-bold whitespace-nowrap transition-all ${activeTab === 'booking' ? 'bg-primary text-white' : 'text-gray-500 hover:bg-white'}`}>سياسات الحجز</button>
                <button onClick={() => setActiveTab('payment')} className={`px-4 py-2 rounded-lg text-xs font-bold whitespace-nowrap transition-all ${activeTab === 'payment' ? 'bg-primary text-white' : 'text-gray-500 hover:bg-white'}`}>بوابات الدفع</button>
                <button onClick={() => setActiveTab('footer')} className={`px-4 py-2 rounded-lg text-xs font-bold whitespace-nowrap transition-all ${activeTab === 'footer' ? 'bg-primary text-white' : 'text-gray-500 hover:bg-white'}`}>الفوتر والتطبيق</button>
            </div>
            <Button onClick={handleSave} disabled={saving} className="h-11 px-8 rounded-xl font-bold gap-2 shadow-lg shadow-primary/20 bg-gray-900 text-white hover:bg-black">
