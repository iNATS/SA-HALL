...
import { 
  MapPin, CheckCircle2, Loader2, ArrowRight, Package, Info, Sparkles, Check, 
  FileText, Plus, Minus, CreditCard, ShoppingBag, Phone, User, MessageCircle, Moon, Tag, Users
} from 'lucide-react';
...
export const HallDetails: React.FC<HallDetailsProps> = ({ item, user, onBack, onPay, onNavigate }) => {
  const [bookingMode, setBookingMode] = useState<'package' | 'night'>('package');
  ...
  useEffect(() => {
    ...
    } else if (item.price_per_night > 0) {
        setBookingMode('night');
        setSelectedPackage(null);
        setGuestCounts({ men: 50, women: 50 });
    }
    ...
    fetchConfig();
  }, [item.id, item.packages, item.price_per_night]);
  ...
  const priceDetails = useMemo(() => {
      ...
      } else if (bookingMode === 'night') {
          baseCost = Number(item.price_per_night) || 0;
      }
      ...
      const adjustedBaseCost = baseCost * seasonalMultiplier;
      const addonsTotal = selectedAddons.reduce((sum, a) => sum + Number(a.price), 0);
      const subTotal = adjustedBaseCost + addonsTotal;
      ...
      return { 
          ...
          total: subTotal // Original total before discount/VAT
      };
  }, [selectedPackage, bookingMode, bookingDate, guestCounts, item.seasonal_prices, selectedAddons, appliedCoupon, item.price_per_night]);
  ...
  const handlePaymentClick = async () => {
      ...
      if (bookingMode === 'package' && selectedPackage) {
          ...
          // No validation needed for capacity here as it's a fixed night price, unless specific restriction logic is desired.
          // But if we want to ensure they don't overbook capacity even in Night mode (if we had input):
          // Since we hide inputs in night mode, we assume full capacity is used.
      }
      ...
      try {
          ...
          const bookingPayload = {
              ...
              package_details: bookingMode === 'package' ? selectedPackage : { name: 'حجز كامل (سعر الليلة)', price: item.price_per_night },
              ...
              guests_adults: bookingMode === 'package' ? guestCounts.men : item.capacity_men,
              guests_children: bookingMode === 'package' ? guestCounts.women : item.capacity_women,
              items: [
                  ...(bookingMode === 'package' && selectedPackage ? [{ name: `باقة: ${selectedPackage.name}`, price: selectedPackage.price, qty: totalGuests, type: 'package' }] : []),
                  ...(bookingMode === 'night' ? [{ name: 'حجز القاعة بالكامل (سعر الليلة)', price: item.price_per_night, qty: 1, type: 'package' }] : []),
                  ...selectedAddons.map(addon => ({ name: addon.name, price: addon.price, qty: 1, type: 'addon' }))
              ]
          };

          const { error } = await supabase.from('bookings').insert([bookingPayload]).select().single();
            ...
            <div className="lg:col-span-2 space-y-8">
                ...
                <div className="bg-white border border-gray-100 rounded-[2.5rem] p-8 shadow-sm">
                    ...
                    <div className="flex flex-nowrap gap-4 overflow-x-auto pb-4 no-scrollbar">
                        ...
                        {/* Option: Price Per Night - Conditionally Rendered */}
                        {item.price_per_night > 0 && (
                            <div 
                                onClick={() => { setBookingMode('night'); setSelectedPackage(null); }}
                                className={`cursor-pointer border-2 rounded-[2rem] p-6 transition-all relative overflow-hidden min-w-[280px] flex-1 ${bookingMode === 'night' ? 'border-primary bg-primary/5' : 'border-gray-100 hover:border-gray-200'}`}
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <h4 className="font-black text-lg flex items-center gap-2"><Moon className="w-5 h-5 text-indigo-500" /> سعر الليلة</h4>
                                    {bookingMode === 'night' && <CheckCircle2 className="w-6 h-6 text-primary" />}
                                </div>
                                <div className="space-y-2 text-xs font-bold text-gray-500 mb-4">
                                    <p>حجز القاعة بالكامل بدون التقيد بعدد الأفراد (حسب السعة).</p>
                                    <div className="flex justify-between bg-white p-2 rounded-lg border border-gray-100 mt-2">
                                        <span>السعة القصوى</span>
                                        <span>{item.capacity} شخص</span>
                                    </div>
                                </div>
                                <div className="flex items-end gap-1">
                                    <PriceTag amount={item.price_per_night} className="text-xl font-black text-primary block text-left" />
                                    <span className="text-[10px] text-gray-400 font-bold mb-1">/ لليلة</span>
                                </div>
                            </div>
                        )}
                        ...
                        {item.packages?.map((pkg, idx) => (
                            <div 
                                key={idx} 
                                onClick={() => { setBookingMode('package'); setSelectedPackage(pkg); setGuestCounts({ men: pkg.min_men, women: pkg.min_women }); }}
                                className={`cursor-pointer border-2 rounded-[2rem] p-6 transition-all relative overflow-hidden min-w-[280px] flex-1 ${bookingMode === 'package' && selectedPackage?.name === pkg.name ? 'border-primary bg-primary/5' : 'border-gray-100 hover:border-gray-200'}`}
                            >
                                <div className="flex justify-between items-start mb-4">
                                    <h4 className="font-black text-lg">{pkg.name}</h4>
                                    {bookingMode === 'package' && selectedPackage?.name === pkg.name && <CheckCircle2 className="w-6 h-6 text-primary" />}
                                </div>
                                <div className="space-y-2 text-xs font-bold text-gray-500 mb-4">
                ...
                <div className="sticky top-28 bg-white border border-gray-200 rounded-[2.5rem] p-6 space-y-6">
                    ...
                    {showSuccess ? (
                        ...
                        <>
                            ...
                            {/* Guest Counts & Pricing Logic - UPDATED */}
                            <div className="space-y-4 border-t border-gray-50 pt-4">
                                <div className="flex justify-between items-center">
                                    <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest">
                                        {bookingMode === 'package' ? 'الباقة المختارة' : 'نوع الحجز'}
                                    </label>
                                    <span className="text-xs font-black text-primary bg-primary/5 px-2 py-1 rounded-lg">
                                        {bookingMode === 'package' ? selectedPackage?.name : 'حجز كامل (سعر الليلة)'}
                                    </span>
                                </div>
                                {bookingMode === 'night' ? (
                                    <div className="bg-gray-50 p-4 rounded-2xl border border-gray-100 text-center space-y-2">
                                        <p className="text-[10px] font-bold text-gray-400 uppercase tracking-widest mb-1">السعة الاستيعابية</p>
                                        <div className="flex justify-center items-center gap-4">
                                            <div className="text-center">
                                                <span className="block text-lg font-black text-gray-900">{item.capacity_men || 0}</span>
                                                <span className="text-[9px] font-bold text-gray-400">رجال</span>
                                            </div>
                                            <div className="h-8 w-px bg-gray-200"></div>
                                            <div className="text-center">
                                                <span className="block text-lg font-black text-gray-900">{item.capacity_women || 0}</span>
                                                <span className="text-[9px] font-bold text-gray-400">نساء</span>
                                            </div>
                                            <div className="h-8 w-px bg-gray-200"></div>
                                            <div className="text-center">
                                                <span className="block text-lg font-black text-primary">{item.capacity}</span>
                                                <span className="text-[9px] font-bold text-gray-400">إجمالي</span>
                                            </div>
                                        </div>
                                    </div>
                                ) : (
