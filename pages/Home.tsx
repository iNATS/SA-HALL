...
import { Hall, UserProfile, Service, SAUDI_CITIES, PromoConfig, ThemeConfig } from '../types';
import { Button } from '../components/ui/Button';
import { PriceTag } from '../components/ui/PriceTag';
import { Modal } from '../components/ui/Modal';
import { 
  Sparkles, Star, MapPin, Zap, ArrowLeft, ShoppingBag, Store, Search, Users, Calendar, Building2, Palmtree, ArrowRight, X
} from 'lucide-react';
...
export const Home: React.FC<HomeProps> = ({ user, onLoginClick, onRegisterClick, onBrowseHalls, onNavigate, onLogout, logoUrl }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      if (settingsData?.value?.promo_config) {
          const config = settingsData.value.promo_config as PromoConfig;
          if (config.show) {
      ...
      // Fetch Theme Config
      if (settingsData?.value?.theme_config) {
          setThemeConfig(settingsData.value.theme_config as ThemeConfig);
      }
  ...
  const handlePromoClick = () => {
      if (promoConfig?.link_tab) {
          onNavigate(promoConfig.link_tab);
          setShowPromo(false);
      }
  };
  ...
  return (
    <div className="min-h-screen bg-[#F9FAFB] text-gray-900 font-tajawal">
      ...
      <section className="relative w-full min-h-[90vh] flex items-center justify-center overflow-hidden bg-gray-900">
          ...
          <div className="relative z-10 w-full max-w-7xl mx-auto px-6 grid lg:grid-cols-2 gap-16 items-center pt-24 pb-12">
              ...
              <div className="animate-in slide-in-from-left duration-1000 delay-200">
                  <div className="bg-white/10 backdrop-blur-xl border border-white/20 p-4 rounded-[2rem] shadow-2xl max-w-md">
                      <div className="bg-white rounded-[1.5rem] p-6 shadow-inner">
                          <div className="flex justify-center mb-6">
                              <img src={themeConfig?.logoUrl || logoUrl || "https://dash.hall.sa/logo.svg"} alt="Logo" className="h-20 w-auto object-contain" />
                          </div>

                          <div className="flex bg-gray-50 p-1 rounded-xl border border-gray-100 mb-4">
                              <button onClick={() => setSearchTab('halls')} className={`flex-1 py-2 rounded-lg text-xs font-black transition-all flex items-center justify-center gap-2 ${searchTab === 'halls' ? 'bg-white shadow-sm text-primary' : 'text-gray-400 hover:text-gray-600'}`}>
                                  <Building2 className="w-3 h-3" /> القاعات
                              </button>
                              <button onClick={() => setSearchTab('services')} className={`flex-1 py-2 rounded-lg text-xs font-black transition-all flex items-center justify-center gap-2 ${searchTab === 'services' ? 'bg-white shadow-sm text-primary' : 'text-gray-400 hover:text-gray-600'}`}>
                                  <Sparkles className="w-3 h-3" /> الخدمات
                              </button>
                          </div>
                          ...
                          <form onSubmit={handleHeroSearch} className="space-y-4">
                              <div className="space-y-1">
                                  ...
                                  <div className="relative">
                                      <select 
                                          ...
                                          className="w-full h-12 bg-gray-50 border border-gray-100 rounded-xl px-4 text-sm font-bold outline-none focus:ring-2 focus:ring-primary/20 transition-all appearance-none text-gray-700 cursor-pointer"
                                      >
                              ...
                              <div className="space-y-1">
                                  <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">تاريخ المناسبة</label>
                                  <div className="relative">
                                      <input 
                                          type="date"
                                          value={searchFilters.date}
                                          onChange={(e) => setSearchFilters({...searchFilters, date: e.target.value})}
                                          className="w-full h-12 bg-gray-50 border border-gray-100 rounded-xl px-4 text-sm font-bold outline-none focus:ring-2 focus:ring-primary/20 transition-all text-gray-700"
                                      />
                                  </div>
                              </div>
                              ...
                              {searchTab === 'halls' && (
                                  <div className="space-y-1">
                                      <label className="text-[10px] font-black text-gray-400 uppercase tracking-widest px-1">عدد الضيوف المتوقع</label>
                                      <div className="grid grid-cols-2 gap-2">
                                          <div className="relative">
                                              <input 
                                                  type="number" 
                                                  placeholder="رجال" 
                                                  className="w-full h-12 bg-gray-50 border border-gray-100 rounded-xl px-4 text-sm font-bold outline-none focus:ring-2 focus:ring-primary/20 text-center"
                                                  value={searchFilters.menCount}
                                                  onChange={(e) => setSearchFilters({...searchFilters, menCount: e.target.value})}
                                              />
                                              <Users className="absolute left-4 top-1/2 -translate-y-1/2 text-gray-300 w-3 h-3" />
                                          </div>
                                          <div className="relative">
                                              <input 
                                                  type="number" 
                              ...
                              <Button 
                                  type="submit" 
                                  className="w-full h-12 rounded-xl font-black text-base bg-gray-900 text-white shadow-lg hover:bg-black hover:scale-[1.02] transition-all flex items-center justify-center gap-2 mt-4"
                              >
      ...
      {/* REDESIGNED PROMO POPUP */}
      {showPromo && promoConfig && (
          <Modal 
            isOpen={showPromo} 
            onClose={() => setShowPromo(false)} 
            title="" 
            className="max-w-md p-0 overflow-hidden border-none bg-transparent shadow-2xl rounded-[3rem]"
            hideHeader={true}
            noPadding={true}
          >
              <div className="relative w-full aspect-[3/4] rounded-[3rem] overflow-hidden group">
                  {/* Full Background Image */}
                  <img 
                    src={promoConfig.image_url} 
                  ...
                  <button 
                    ...
                    className="absolute top-4 left-4 z-50 w-10 h-10 bg-white/20 hover:bg-white/30 backdrop-blur-md rounded-full flex items-center justify-center text-white transition-all shadow-lg border border-white/10"
                  >
                      <X className="w-5 h-5" />
                  </button>

                  {/* Content at Bottom */}
                  <div className="absolute bottom-0 left-0 right-0 p-8 text-center text-white z-10 flex flex-col items-center gap-4">
                      <div className="space-y-2">
                          <h2 className="text-3xl font-black leading-tight drop-shadow-md">{promoConfig.title}</h2>
                          {promoConfig.description && (
                              <p className="text-white/80 font-bold text-sm leading-relaxed max-w-xs mx-auto drop-shadow-sm">
                                  {promoConfig.description}
                              </p>
                          )}
                      </div>
                      
                      {promoConfig.link_tab && (
                          <Button 
                            onClick={handlePromoClick} 
                            className="w-full h-14 rounded-2xl font-black text-lg bg-primary text-white hover:bg-primary/90 shadow-xl mt-2 pointer-events-auto"
                          >
                              تصفح العرض الآن
                          </Button>
                      )}
