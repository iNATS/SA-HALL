...
export const VendorAuth: React.FC<VendorAuthProps> = ({ isLogin = false, onRegister, onLogin, onDataChange, onNavigate, onBack }) => {
  ...
  // Force clear session when entering register mode to prevent "New password should be different" error
  useEffect(() => {
      const clearSession = async () => {
          if (!isLogin) {
              try {
                ...
                // Silently fail if token missing
              }
          }
      };
      clearSession();
  ...
  const handleRegisterStep3 = async (e: React.FormEvent) => {
      ...
      try {
          ...
          // Graceful handling: If user is re-registering an existing email with same password, proceed.
          if (err.message && (
              err.message.includes("New password should be different from the old password") ||
              err.message.includes("Password should be different")
          )) {
              toast({ title: 'تم إنشاء الحساب', description: 'جاري توجيهك لاختيار نوع النشاط...', variant: 'success' });
              if (onRegister) onRegister();
          } else {
                    ...
                    {mode === 'login' ? (
                        <>
                            <div className="flex flex-col gap-2 text-sm font-bold text-primary">
                                <button onClick={() => { if (onNavigate) onNavigate('forgot_password'); }} className="hover:underline">نسيت كلمة المرور</button>
