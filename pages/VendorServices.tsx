...
export const VendorServices: React.FC<VendorServicesProps> = ({ user }) => {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      const [servicesRes, categoriesRes, settingsRes] = await Promise.all([
        ...
        supabase.from('system_settings').select('value').eq('key', 'platform_config').maybeSingle()
      ]);
      setServices(servicesRes.data || []);
      setCategories(categoriesRes.data || []);
      if(settingsRes.data?.value?.service_listing_fee) setServiceFee(settingsRes.data.value.service_listing_fee);
    } catch (err: any) {
  ...
  const handlePayUpgrade = async () => {
      setUpgrading(true);
      try {
          await new Promise(r => setTimeout(r, 1500)); // Sim payment
          await supabase.from('profiles').update({ service_limit: user.service_limit + 1 }).eq('id', user.id);
          
          await supabase.from('external_invoices').insert([{
              vendor_id: user.id,
              customer_name: user.business_name,
              total_amount: serviceFee,
              vat_amount: serviceFee * 0.15,
              status: 'paid',
              items: [{ description: 'إضافة خدمة جديدة', quantity: 1, unit_price: serviceFee, total: serviceFee }]
          }]);

          toast({ title: 'تم الدفع بنجاح', description: 'يمكنك الآن إضافة خدمة جديدة.', variant: 'success' });
          setShowUpgradeModal(false);
          // Directly open modal for UX
          setCurrentService({ is_active: true, category: categories[0]?.name || '', images: [], service_areas: [] }); 
          setIsEditing(true);
  ...
  const handleFileUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      setCurrentService(prev => ({ ...prev, images: newImages, image_url: newImages[0] }));
    ...
    <div className="space-y-8 pb-10 font-sans text-right">
      <div className="flex justify-between items-center bg-white p-6 rounded-[2rem] border border-gray-100">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1 font-bold">الحد المسموح: {services.length} / {user.service_limit}</p>
        </div>
      ...
      <Modal isOpen={showUpgradeModal} onClose={() => setShowUpgradeModal(false)} title="ترقية الباقة">
          <div className="text-center space-y-6 p-4">
              ...
              <div>
                  ...
                  <p className="text-gray-500 font-bold mt-2">لإضافة خدمة جديدة، يجب دفع رسوم الاشتراك الإضافي.</p>
              </div>
              <div className="bg-gray-50 p-6 rounded-2xl border border-gray-100">
                  <p className="text-xs font-bold text-gray-400 uppercase">قيمة الاشتراك</p>
                  <PriceTag amount={serviceFee} className="text-3xl font-black text-primary justify-center mt-2" />
              </div>
              <Button onClick={handlePayUpgrade} disabled={upgrading} className="w-full h-14 rounded-2xl font-black text-lg bg-gray-900 text-white">
                  {upgrading ? <Loader2 className="animate-spin" /> : 'دفع وإضافة الخدمة'}
              </Button>
          </div>
      </Modal>
        ...
        <div className="space-y-6 text-right">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-2">
                ...
                <select className="w-full h-12 border rounded-xl px-4" value={currentService.category || ''} onChange={e => setCurrentService({...currentService, category: e.target.value})}>
                {categories.map(cat => <option key={cat.id} value={cat.name}>{cat.name}</option>)}
                </select>
            </div>
            <Input label="السعر" type="number" value={currentService.price || ''} onChange={e => setCurrentService({...currentService, price: Number(e.target.value)})} className="h-12 rounded-xl" />
          </div>
          <div className="space-y-2">
          ...
          <div className="pt-4 border-t border-gray-100">
            <div onClick={() => fileInputRef.current?.click()} className="w-full h-24 rounded-xl bg-purple-50 border-2 border-dashed border-purple-200 flex items-center justify-center cursor-pointer hover:bg-purple-100 transition-all">
                {uploading ? <Loader2 className="w-6 h-6 animate-spin" /> : <div className="flex items-center gap-2 text-primary font-bold"><Plus className="w-5 h-5" /> رفع صور</div>}
            </div>
            <input type="file" hidden ref={fileInputRef} accept="image/*" onChange={handleFileUpload} />
          </div>

          <Button onClick={handleSave} disabled={saving} className="w-full h-12 rounded-xl font-black mt-4">
