...
import { 
  ...
  Receipt, Store, LogOut, UserCheck, ShieldCheck, Sparkles
} from 'lucide-react';
...
export const Sidebar: React.FC<SidebarProps> = ({ user, activeTab, setActiveTab, onLogout, isOpen, setIsOpen, platformLogo }) => {
  ...
  let menuGroups: { title: string; items: { id: string; label: string; icon: React.ReactNode }[] }[] = [];
    ...
    menuGroups = [
      {
        title: "الرئيسية",
        items: [
          { id: 'admin_dashboard', label: 'لوحة التحكم', icon: <BarChart3 className="w-5 h-5" /> },
          { id: 'admin_requests', label: 'الطلبات والاشتراكات', icon: <Inbox className="w-5 h-5" /> },
        ]
      },
      {
        title: "إدارة المستخدمين",
        items: [
          { id: 'admin_vendors', label: 'أصحاب القاعات', icon: <Building2 className="w-5 h-5" /> },
          { id: 'admin_customers', label: 'العملاء', icon: <Users className="w-5 h-5" /> },
        ]
      },
      {
        title: "المتجر والمالية",
        items: [
          { id: 'admin_store', label: 'إدارة المتجر', icon: <Store className="w-5 h-5" /> }, 
          { id: 'admin_accounting', label: 'الحسابات والتقارير', icon: <Receipt className="w-5 h-5" /> },
        ]
      },
      {
    ...
    menuGroups = [
      ...
      },
      {
        ...
        items: [
          ...
          { id: 'vendor_clients', label: 'قاعدة العملاء', icon: <UserCheck className="w-5 h-5" /> },
        ]
      },
      {
        title: "الإعدادات",
