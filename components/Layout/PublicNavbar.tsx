...
import { 
  ...
  Building2, Palmtree, Sparkles, ShoppingBag, LogIn, UserPlus, Ticket
} from 'lucide-react';
...
export const PublicNavbar: React.FC<PublicNavbarProps> = ({ 
  user, onLoginClick, onRegisterClick, onNavigate, onLogout, activeTab, logoUrl 
}) => {
  ...
  const navItems = [
    ...
    { id: 'browse_halls', label: 'القاعات', icon: <Building2 className="w-4 h-4" /> },
    { id: 'browse_services', label: 'الخدمات', icon: <Sparkles className="w-4 h-4" /> },
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-full">
          <div className="flex items-center justify-between h-full">
            ...
            <div className="flex items-center gap-8">
              ...
              >
                <img src={logoUrl || "https://dash.hall.sa/logo.svg"} alt="SA Hall" className="h-24 w-auto object-contain transition-transform hover:scale-105" />
              </div>
              ...
              {!user ? (
                <div className="flex items-center gap-2">
                  <button 
                    ...
                    className="px-4 h-11 text-sm font-bold text-gray-500 hover:text-primary hover:bg-gray-50 rounded-xl transition-all flex items-center gap-2 hidden md:flex border border-transparent hover:border-gray-100"
                  >
                  ...
                  {isUserMenuOpen && (
                    <div className="absolute left-0 mt-3 w-56 bg-white border border-gray-100 rounded-2xl ring-1 ring-black/5 overflow-hidden animate-in fade-in slide-in-from-top-2 z-[110] text-right shadow-xl">
                      ...
                      <div className="p-1.5 space-y-0.5">
                        <button onClick={handleDashboardClick} className="w-full flex items-center justify-start gap-3 px-3 py-2.5 rounded-xl text-xs font-bold text-gray-600 hover:bg-gray-50 hover:text-primary transition-colors">
                ...
                <nav className="flex flex-col gap-2">
                    ...
                    {!user && (
                        <div className="pt-6 mt-2 border-t border-gray-100 space-y-3">
                           <button 
                                onClick={() => { onNavigate('guest_login'); setIsMobileMenuOpen(false); }} 
                                className="w-full h-14 rounded-2xl font-bold bg-gray-50 text-gray-700 flex items-center justify-center gap-2 hover:bg-gray-100"
                            >
                                <Ticket className="w-4 h-4" /> متابعة الحجز
                            </button>
                            <button 
